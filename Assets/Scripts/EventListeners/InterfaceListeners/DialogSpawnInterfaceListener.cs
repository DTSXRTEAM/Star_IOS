...
public class DialogSpawnInterfaceListener : CPEventListener {

	public GameObject DefaultDialogPrefab;
	public GameObject SuccessDialogPrefab;
	...
	{
		if (par.GetType() == typeof(SimulatorInterface.Interface))
		{
			var intface = (SimulatorInterface.Interface)par;
			...
			if (intface.error > 0)
			{
				...
				if (AllreadyShowing(intface.error))
					return;

				//CloseCurrentDialog();

				var entry = DialogTable.Instance.GetTableEntry(intface.error);

				// Is success dialog
				if (!string.IsNullOrWhiteSpace(entry.Type) && entry.Type.ToLower() == "success")
				{
					CurrentDialog = Instantiate(SuccessDialogPrefab);
				}				
				else
				{
				...
				{
					buttonScript.ErrorNo = intface.error;
				}
